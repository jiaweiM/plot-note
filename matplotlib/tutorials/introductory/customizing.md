...
- 更改绘图颜色，需要修改 axes 的 `prop_cycle` 属性
...
- `matplotlib.rc` 可以一次修改通过 rc 参数，使用关键字参数
...
`matplotlib.rc_context` 也可以用作装饰器，修改 matplotlib 在函数中的默认值：
...
`matplotlib.rcdefaults` 将恢复设置为 Matplotlib 默认设置。
...
rcParams 完整参数列表可参考 [matplotlib.rcParams](https://matplotlib.org/stable/api/matplotlib_configuration_api.html#matplotlib.rcParams)，样式表中忽略的 rcParams 可参考 [matplotlib.style.use](https://matplotlib.org/stable/api/style_api.html#matplotlib.style.use)。

Matplotlib 提供了许多[预定义的样式](https://matplotlib.org/stable/gallery/style_sheets/style_sheets_reference.html)。例如，"ggplot" 模仿了 R 的 ggplot 样式，使用方式：
...
例如，创建包含如下内容的样式表 `./images/presentation.mplstyle`：
...
也可以将样式表 `<style-name>.mplstyle` 放在 `mpl_configdir/stylelib` 目录。这样就能直接调用 `style.use(<style-name>)` 使用样式表。

`mpl_configdir` 默认位置为 `~/.config/matplotlib`，也可以调用 `matplotlib.get_configdir()` 查看默认位置，如果该目录不存在，就需要自己手动创建。也可以通过设置 `MPLCONFIGDIR` 环境变量来默认目录。

> **NOTE** 如果自定义样式表与 matplotlib 自带样式表同名，自定义样式表会覆盖同名样式表。

将自定义样式表 `<style-name>.mplstyle` 放到 `mpl_configdir/stylelib` 目录后，就可以直接使用：
...
1. 当前工作目录，一般用在特定地方的自定义
2. `$MATPLOTLIBRC` 或 `$MATPLOTLIBRC/matplotlibrc`
3. 根据系统不同，在特定位置查找：
   - 对 Linux 或 FreeBSD，在 `.config/matplotlib/matplotlibrc` 
   - 其他平台为 `.matplotlib/matplotlibrc`
4. `INSTALL/matplotlib/mpl-data/matplotlibrc`

找到 `matplotlibrc` 文件后，Matplotlib 就不继续在其他位置寻找。

显示当前使用的 `matplotlibrc` 文件：
